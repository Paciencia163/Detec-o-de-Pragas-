...
def show(model):
    st.markdown("<h1 style='text-align:center;'>🎞️ Detecção de Vídeo</h1>", unsafe_allow_html=True)
    st.markdown("Faça upload de um vídeo para detectar pragas. As informações sobre pragas aparecerão após a reprodução.")
    ...
    if uploaded_video:
        ...
        stframe = st.empty()
        st.info("🔄 Reproduzindo vídeo com detecção ao vivo...")
        ...
        cap.release()
        st.success("✅ Vídeo concluído. Exibindo informações sobre pragas...")
        ...
        if detected_pests:
            for pest, score in detected_pests.items():
                ...
                if not row.empty:
                    ...
                    st.warning("Nenhuma informação adicional encontrada no CSV.")
                st.markdown("---")
        else:
            st.info("✅ Nenhuma praga detectada com confiança acima de 70%.")
